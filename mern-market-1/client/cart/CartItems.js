...
export default function CartItems (props) {
  ...
  const openCheckout = () => {
    ...
    <Card className={classes.card} elevation={24}> cart/CarItems
      ...
      {cartItems.length>0 ? (<span>
              ...
              <div className={classes.details}>
                ...
                <div className={classes.subheading}>
                  Cantidad: <TextField
                              ...
                              margin="normal"/>
                            <Button className={classes.removeButton} color="primary" onClick={removeItem(i)}>Eliminar</Button>
                </div>
              </div>
            </Card>
            <Divider/>
        ...
        <div className={classes.checkout}>
          ...
          {!props.checkout && (auth.isAuthenticated()?
            <Button color="secondary" variant="contained" onClick={openCheckout}>Pasa por caja</Button>
            :
            <Link to="/signin">
              <Button color="primary" variant="contained">Sign in to checkout</Button>
            </Link>)}
          <Link to='/' className={classes.continueBtn}>
            <Button variant="contained">Continua comprando</Button>
          </Link>
        </div>
      </span>) :
      <Typography variant="subtitle1" component="h3" color="primary">No hay productos en el carrito</Typography>
    }
    </Card>)
}
